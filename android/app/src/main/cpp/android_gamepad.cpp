 ...
 *
 * The on-screen gamepad (com.yestermester.simpsons.gamepad.*) sends its state
...
Sint16 TriggerRaw(float value) {
  ...
  return static_cast<Sint16>(std::lround(std::clamp(value, 0.0f, 1.0f) * 65535.0f) - 32768);
}
...
bool AttachLocked() {
  ...
  desc.name = "The Simpsons Game Touch Pad";

  g_instance = SDL_AttachVirtualJoystick(&desc);
  if (!g_instance) {
    REXLOG_ERROR("virtual gamepad: SDL_AttachVirtualJoystick failed: {}", SDL_GetError());
    return false;
  }
...
Java_com_yestermester_simpsons_gamepad_PadInputBridge_nativeSetButton(
    JNIEnv* /*env*/, jclass /*clazz*/, jint button, jboolean down) {
  std::lock_guard<std::mutex> guard(simpsons::gamepad::g_mutex);
  if (!simpsons::gamepad::g_joystick || button < 0 ||
      button >= simpsons::gamepad::kNumButtons) {
    return JNI_FALSE;
  }
  SDL_SetJoystickVirtualButton(simpsons::gamepad::g_joystick,
                               static_cast<int>(button), down == JNI_TRUE);
  return JNI_TRUE;
}

JNIEXPORT jboolean JNICALL
Java_com_yestermester_simpsons_gamepad_PadInputBridge_nativeSetStick(
    JNIEnv* /*env*/, jclass /*clazz*/, jint stick, jfloat x, jfloat y) {
  std::lock_guard<std::mutex> guard(simpsons::gamepad::g_mutex);
  if (!simpsons::gamepad::g_joystick || stick < 0 || stick > 1) {
    return JNI_FALSE;
  }
  const int base = (stick == 0) ? 0 : 2;
  SDL_SetJoystickVirtualAxis(simpsons::gamepad::g_joystick, base + 0,
                             simpsons::gamepad::StickRaw(x));
  SDL_SetJoystickVirtualAxis(simpsons::gamepad::g_joystick, base + 1,
                             simpsons::gamepad::StickRaw(y));
  return JNI_TRUE;
}

JNIEXPORT jboolean JNICALL
Java_com_yestermester_simpsons_gamepad_PadInputBridge_nativeSetTrigger(
    JNIEnv* /*env*/, jclass /*clazz*/, jint trigger, jfloat value) {
  std::lock_guard<std::mutex> guard(simpsons::gamepad::g_mutex);
  if (!simpsons::gamepad::g_joystick || trigger < 0 || trigger > 1) {
    return JNI_FALSE;
  }
  const int axis = (trigger == 0) ? 4 : 5;
  SDL_SetJoystickVirtualAxis(simpsons::gamepad::g_joystick, axis,
                             simpsons::gamepad::TriggerRaw(value));
  return JNI_TRUE;
}

JNIEXPORT jboolean JNICALL
Java_com_yestermester_simpsons_gamepad_PadInputBridge_nativeIsAttached(
    JNIEnv* /*env*/, jclass /*clazz*/) {
  std::lock_guard<std::mutex> guard(simpsons::gamepad::g_mutex);
  return simpsons::gamepad::g_joystick ? JNI_TRUE : JNI_FALSE;
}
