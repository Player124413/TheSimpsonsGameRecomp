...
import com.yestermester.simpsons.gamepad.PadSettings;
import com.yestermester.simpsons.gamepad.VirtualPadView;
 ...
 * packaged). org.libsdl.app.* sources are copied from the SDK's SDL3
 * submodule by scripts/setup-android.sh.
 */
public class MainActivity extends SDLActivity {

    /** The on-screen virtual gamepad overlay (created when enabled). */
    ...
    protected void onResume() {
        super.onResume();
        if (mGamepad != null) {
            // Settings may have changed while paused (or the pad was enabled
            // from the setup screen); re-apply visibility.
            mGamepad.applyEnabledState();
        } else if (PadSettings.get(this).enabled()) {
            mGamepad = VirtualPadView.install(this);
        }
    }
